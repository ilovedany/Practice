...
namespace Task_7.Services
{
    internal class SpecialistRankService : ISpecialistRankService
    {
        ...
        public SpecialistRank GetRankToIDAsync(int idRank)
        {
            using (var db = new SqlConnection(_connectionString))
            {
                var sqlQuery = "SELECT * FROM Specialist_rank WHERE Id = @id_rank";
                
                return db.QueryFirstOrDefault<SpecialistRank>(sqlQuery, new { id_rank = idRank });
